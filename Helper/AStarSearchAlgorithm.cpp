                                        ...
                                        const std::pair<int, int>& dest)
{
    ...
    while (!openList.empty())
    {
        ...
        else if (!closedList[i - 1][j] && IsUnblockedLevelMap(levelMap, i - 1, j))
        {
        ...
        else if (!closedList[i][j + 1] && IsUnblockedLevelMap(levelMap, i, j + 1))
        {
            gNew = cellDetails[i][j].g + 1.0;
...
{
    return characterMap[i][j] == MapDefinitions::eEmpty;
}
